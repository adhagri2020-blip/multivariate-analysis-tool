# ---- ALL IMPORTS (removed subprocess and google.colab) ----
import pandas as pd
import numpy as np
...
from sklearn.metrics import confusion_matrix, classification_report, roc_curve, auc, silhouette_score, silhouette_samples
...
# ---- ANALYSIS FUNCTIONS (all 11 methods) ----
# 1. CORRELATION
def correlation_analysis(data, vars_list, corr_method='pearson', alpha=0.05, threshold=0.5):
    ...
    strong = (sig_df[abs(sig_df['Corr']) > threshold] if not sig_df.empty else pd.DataFrame())
...
# 2. REGRESSION
def regression_analysis(data, y_var, x_vars, reg_type='multiple', test_size=0.2, poly_deg=2, ridge_alpha=1.0):
    ...
    if reg_type == 'simple' and len(x_vars) == 1:
        ...
        eq = f"{y_var} = {model.params.values[0]:.3f} + {model.params.values[1]:.3f}*{x_vars[0]}"
        ...
        coef_df = pd.DataFrame({'Variable': ['Intercept'] + [f'poly_{i}' for i in range(X_tr_p.shape[1])],
                                'Coef': model.params.values})
        r2 = model.rsquared
        r2_adj = model.rsquared_adj
        ...
        eq = f"{y_var} = {ridge.intercept_:.3f} + " + " + ".join([f"{c:.3f}*{v}" for v, c in zip(x_vars, ridge.coef_)])
        ...
        r2_adj = 1 - (1 - r2) * (len(y_train) - 1) / (len(y_train) - len(x_vars) - 1)
        eq = f"{y_var} = {lasso.intercept_:.3f} + " + " + ".join([f"{c:.3f}*{v}" for v, c in zip(x_vars, lasso.coef_)])
    
    else:  # multiple linear
        X_train_sm = sm.add_constant(X_train)
        model = sm.OLS(y_train, X_train_sm).fit()
        y_pred_test = model.predict(sm.add_constant(X_test))
        y_pred_train = model.predict(X_train_sm)
        ...
        r2 = model.rsquared
        r2_adj = model.rsquared_adj
        eq = f"{y_var} = {model.params.values[0]:.3f} + " + " + ".join([f"{c:.3f}*{v}" for v, c in zip(x_vars, model.params.values[1:])])
    ...
    if len(x_vars) >= 2:
        ...
        vif_df = pd.DataFrame({'Variable': x_vars, 'VIF': ['N/A — single predictor']})
...
def hierarchical_clustering(data, vars_list, linkage_method='ward', dist_metric='euclidean', n_clusters=3):
    ...
    ax.set_title('Cluster Scatter (PCA)')
...
def discriminant_analysis(data, x_vars, y_var, da_type='lda', test_size=0.2):
    ...
    model = LinearDiscriminantAnalysis() if da_type == 'lda' else QuadraticDiscriminantAnalysis()
    ...
    if da_type == 'lda':
        ...
        fig3, ax = plt.subplots()
        coef_df.sort_values('Coefficient').plot.barh(x='Variable', y='Coefficient', ax=ax)
...
def factor_analysis(data, vars_list, n_factors='auto', rotation='varimax'):
    ...
    for i in range(p):
        for j in range(p):
            partial[i, j] = (-R_inv[i, j] / np.sqrt(abs(R_inv[i, i] * R_inv[j, j])))
...
# 10. t-SNE / UMAP
def tsne_umap(data, vars_list, color_var=None, method='tsne', perplexity=30, n_neighbors=15):
    X = data[vars_list].dropna()
    Xs = StandardScaler().fit_transform(X)
    ...
    sil = silhouette_score(emb, colors) if len(np.unique(colors)) > 1 else np.nan
...
def sem_analysis(data, model_spec, observed_vars):
    ...
    try:
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.text(0.5, 0.5, "Path diagram not auto-generated.\nSee parameter table.",
                ha='center', va='center')
        ax.axis('off')
